...
- **Meta** (`meta`) — Facebook Pages + Instagram Business. Configure `META_APP_ID`, `META_APP_SECRET`, `META_REDIRECT_URI`, and `META_TOKEN_ENCRYPTION_KEY`. OAuth: `/api/connectors/meta`, callback: `/api/connectors/meta/callback`.
  - `META_LOGIN_CONFIG_ID` (optional): a Facebook Login for Business configuration ID, for onboarding **external** client businesses only. Meta blocks selecting your own app's Business Portfolio as the client business in a Login Configuration — a business can't use Login for Business to grant access to itself.
  - For Kaan Creative's **own** Pages/Instagram accounts (owned by the same Business Portfolio that owns this app), use the admin-only system-user token flow instead: `POST /api/connectors/meta/system-user` (`connect_meta_system_user` feature, ADMIN only; UI panel on the Accounts page). An admin generates a non-expiring System User access token in Business Settings and pastes it in; the token is validated via `debugMetaToken`/`validateMetaSystemUserToken` (must be valid, issued for this app, non-expiring, and hold `pages_show_list`/`pages_read_engagement`/`instagram_basic`/`instagram_manage_insights`) and never persisted, logged, or returned — only `maskMetaToken`'s short preview is ever shown. Pages/Instagram accounts are discovered via the Business Manager API (`fetchBusinessManagedPages`, `owned_pages` + `client_pages`) using `META_BUSINESS_ID`, not `/me/accounts`. Each Page's own access token is then encrypted (`encryptToken`) and stored per `MetaAccount`, same as the OAuth flow. Ad accounts (`fetchBusinessAdAccounts`) are discovered and shown in the preview for future use but not yet synced — there is no ad-account sync pipeline.
