...
export async function GET(request: NextRequest) {
  ...
  const clients = await db.client.findMany({ where: { active: !archived }, include: { connections: { where: { sourceAccountId: { not: null } }, select: { platform: true, displayName: true, lastSyncedAt: true, lastSuccessfulSyncAt: true, lastFailedSyncAt: true, lastFailureReason: true, tokenExpiresAt: true, sourceAccountId: true, historicalBackfillStatus: true, historicalBackfillProcessedPosts: true, historicalBackfillLastError: true, syncJobs: { orderBy: { createdAt: "desc" }, take: 1, select: { status: true, type: true, attempts: true, runAfter: true, lastError: true } }, syncRuns: { orderBy: { createdAt: "desc" }, take: 3, select: { status: true, startedAt: true, finishedAt: true, postsSynced: true, durationMs: true, errorMessage: true } } } }, _count: { select: { reports: true } } }, orderBy: { name: "asc" } });
