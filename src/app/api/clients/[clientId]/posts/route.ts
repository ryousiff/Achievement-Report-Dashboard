...
export async function GET(request: NextRequest, { params }: { params: Promise<{ clientId: string }> }) {
  ...
  // Only select what the media picker UI (MediaPost) actually renders — this list can be up to 100
  // posts and previously shipped every SocialPost column (including mediaMetadata, metricAvailability
  // state, connectionId, etc.) to the client on every open of the picker.
  const posts = await db.socialPost.findMany({
    where: { connection: { clientId }, publishedAt: { gte: since, ...(until ? { lte: until } : {}) } },
  ...
  const ranked = posts.map((post) => {
    ...
    const { thumbnailStorageKey, mediaSource, ...rest } = post;
    return { ...rest, thumbnailStorageUrl: mediaThumbnailUrl(thumbnailStorageKey), isCollaborative: mediaSource === "COLLABORATIVE", score: (metrics.total_interactions ?? 0) + (metrics.shares ?? 0) + (metrics.saved ?? 0) + (metrics.follows ?? 0) };
  }).sort((left, right) => right.score - left.score);
  const snapshots = await db.socialInsightSnapshot.findMany({ where: { connection: { clientId }, metric: "follower_count", periodType: InsightPeriodType.DAY, periodEnd: { gte: since, ...(until ? { lte: until } : {}) } }, orderBy: { periodEnd: "asc" }, select: { periodEnd: true, value: true } });
