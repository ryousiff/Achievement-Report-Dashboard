...
type ReportPost = { id: string; externalPostId: string; caption: string | null; mediaType: string; mediaUrl: string | null; thumbnailUrl: string | null; permalink: string | null; publishedAt: string; metrics: PostMetrics; score: number };
...
export async function reportPosts(clientId: string, periodStart: Date, periodEnd: Date) {
  ...
  return posts.map((post): ReportPost => {
    ...
    const item = { id: post.id, externalPostId: post.externalPostId, caption: post.caption, mediaType: post.mediaType, mediaUrl: post.mediaUrl, thumbnailUrl: post.thumbnailUrl, permalink: post.permalink, publishedAt: post.publishedAt.toISOString(), metrics, score: 0 };
...
export async function buildStandardReportBlocks(clientId: string, periodStart: Date, periodEnd: Date): Promise<ReportBlock[]> {
  ...
  const hasReach = posts.some((post) => typeof post.metrics.reach === "number");
  ...
  return [
    ...
    { type: BlockType.KPI, title: "أهم الإحصائيات", content: { body: "إحصائيات الفترة المحددة من بيانات Meta المتاحة.", kpis: [kpi("reach", metricLabel.reach, totals.reach.toLocaleString(), hasReach), kpi("views", metricLabel.views, totals.views.toLocaleString()), kpi("engagement-rate", "متوسط التفاعل على أساس الوصول", engagementRate, hasReach), kpi("follows", "المتابعون الجدد", totals.follows.toLocaleString()), kpi("posts", metricLabel.posts, totals.posts.toLocaleString())], comparison: "none", autoFilled: true } },
    { type: BlockType.KPI, title: "التفاعل مع المحتوى", content: { body: "إجماليات التفاعل للمنشورات خلال الفترة.", kpis: [kpi("total_interactions", metricLabel.total_interactions, totals.total_interactions.toLocaleString()), kpi("likes", metricLabel.likes, totals.likes.toLocaleString()), kpi("comments", metricLabel.comments, totals.comments.toLocaleString()), kpi("saved", "حفظ", totals.saved.toLocaleString()), kpi("shares", "مشاركة", totals.shares.toLocaleString())], comparison: "none", autoFilled: true } },
